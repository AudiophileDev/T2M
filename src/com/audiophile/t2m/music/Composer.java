...
public class Composer {
    ...
    public Composer(Sentence[] sentences, float[] avgWordLen) {
        ...
        this.key = new Harmony(sentences[0].getWords()[0].getName().substring(0, 1), Mode.Major, false);
        this.dynamic = DYNAMIC; //sets default dynamic//forte, piano, cresc, decresc
        this.tempo = new Tempo(avgWordLen);
        int i = (this.key.getMode() == Mode.Minor ? 2 : 1);
        this.tempo.setAverageBpm(this.tempo.getAverageBpm() / i);

        this.musicData = new MusicData(tempo, dynamic, key);
    ...
    public Sequence getSequence() {
        ...
        try {
            ...
            trackGenerators[0].writeToTrack(sequence.createTrack(), 0, MyInstrument.Piano);
            trackGenerators[1].writeToTrack(sequence.createTrack(), 9, MyInstrument.Drums); // Channel 10 are drums
